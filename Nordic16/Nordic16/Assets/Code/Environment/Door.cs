...
public class Door : MonoBehaviour {
	...
	void Update () 
	{
        ...
        { 
            ...
            float t_Closed = 0.0f;
            float t_Opened = m_HalfDoorLength * 1.0f;
            
            if (m_IsOpening)
            {
                float t_CurrentHeight = t_Closed + (t_Opened - t_Closed) * t_Factor;
                upperdoor.transform.localPosition = Vector3.up * t_CurrentHeight;
                lowerdoor.transform.localPosition = -Vector3.up * t_CurrentHeight;
            }
            else
            {
